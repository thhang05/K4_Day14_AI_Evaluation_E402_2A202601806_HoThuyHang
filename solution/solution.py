...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{criteria_desc}\n"
            "Score each criterion from 0.0 to 1.0 and reply as JSON, "
            'e.g. {"accuracy": 0.9, "clarity": 0.8}.'
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        all_scores = [
            v for entry in scores_batch for v in entry.get("scores", {}).values()
        ]
        overall_avg = sum(all_scores) / len(all_scores) if all_scores else 0.0

        positional_bias = False
        if len(scores_batch) >= 2:
            first_avg = _avg(scores_batch[0])
            rest_avgs = [_avg(entry) for entry in scores_batch[1:]]
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def _avgs(results: list) -> dict[str, float]:
            n = len(results)
            if not n:
        ...
        new_avg = _avgs(new_results)
        baseline_avg = _avgs(baseline_results)
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant"):
            suggestions.append("Improve prompt clarity and intent detection to keep answers on-topic")
        ...
        default_suggestions = [
            ...
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Review retrieval ranking so relevant context surfaces earlier",
        ]
